...
class prodConfig(ConfigMan):
    dbUrl = os.environ.get('DATABASE_URL').replace("://", "ql://", 1) if os.environ.get('DATABASE_URL') else None
    SQLALCHEMY_DATABASE_URI = dbUrl or 'sqlite:///'+ os.path.join(basdir+ '\SQLA_practice_prod.db')
